	...
	\begin{multicols}{2}
		...
		\begin{figure}[H]
			...
			\begin{tikzpicture}
				...
				\pgfmathsetmacro{\K}{2}
				\pgfmathsetmacro{\T}{.4}
				\pgfmathsetlengthmacro{\zeta}{.15}

				% help macros
				\pgfmathsetlengthmacro{\onemzsq}{sqrt(1 - \zeta^2)}
				\pgfmathsetlengthmacro{\scale}{.8 * \linewidth / 10}

				% axis
				\draw[thick, <->] (.8 * \linewidth,0) node[right] {\(t\)} 
					-- (0,0) -- (0,{2 + \K}) node[above] {\(y\)};

				% plots
				\draw[very thick, hsr-mauve] (0,0) -- ++(0,\K) -- (10,\K);

				\draw[thick, gray, dashed, smooth, samples = 100, variable = \t, domain = 0:10]
					plot ({\t * \scale}, {
						\scale * \K * (1 - exp(-\zeta * \t / \T))
					});

				\draw[very thick, hsr-blue, smooth, samples = 100, variable = \t, domain = 0:10]
					plot ({\t * \scale}, {
						\scale * \K * (
							1 - exp(-\zeta * \t / \T) * (
								cos(\onemzsq * \t / \T * 180 / pi)
								+ \zeta / \onemzsq * sin(\onemzsq * \t / \T * 180 / pi)
							)
						)
					});
